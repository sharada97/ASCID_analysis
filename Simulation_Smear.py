...
save_folder="/data7/CsI_Project/Simulation/Sc46/Figure/Smear/Each"
...
for n,j in enumerate(Energy_Ch):
    ...
    for k in range(0,NDet):
        if j[k]>energy_threshold:            #100keV energy threshold
            ...
            ##Smearing
            mu=j[k]             #closest(bins,j[k])
            sig=sigma_mu_ratio(mu, *popt_new[k][0])*mu
            smearE=float(np.random.normal(mu, sig, 1))
            SmEnergy[k].append(smearE)

            ##Adding efficiency
            event_Energy = smearE
            event_channel = k
            probability= efficiency_function(event_Energy, *popt_eff[event_channel])
            probability1 = probability/eff_max[event_channel]
            #print(probability1)
            select_event = random.choice([event_Energy, -1], p=[probability1, 1-probability1], size=(1))
            if select_event!= -1:
                group_energy.append(event_Energy)
                SmEnergy_eff[event_channel].append(event_Energy)
