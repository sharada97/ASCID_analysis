...
class ASCID_analysis:
    ...
    def __init__(self, data, Data_Time):
        ...
        self.Bins = np.arange(energyThreshold_CsI, energy_end, energy_bin)
    ...
    def coincidence(self):
        ...
        Timed_Energy_event=[]
        for j in range(nEvents):
            if  self.data[1][j]>eventTime+2*timeWindow and self.data[0][j] in csiCh:   #Avoiding center detector overlap
                ...
                while Ev_indx>0 and abs(self.data[1][Ev_indx-1]-eventTime)<=timeWindow:
                    Ev_indx-=1
                    a.extend([Ev_indx])
                Ev_indx=j
                
                while Ev_indx<nEvents-1 and abs(self.data[1][Ev_indx+1]-eventTime)<=timeWindow:
                    Ev_indx+=1    
                    a.extend([Ev_indx])
    ...
    def CoaddedEnergy(self):
        ...
        # f = open('%s/Outputmultiplicity_%d_%d_%dns_%dkeV.pickle'%(save_folder_each, folder_index, index, timeWindow*1e9, energyThreshold_CsI), 'wb')
    ...
    def added_energy(self, group, plot = False):
        ...
        plt.title("%s detector coincidence Spectru"%(group), fontsize=14)
        plt.grid()
        if plot:
    ...
    def exclusive_search(self):
        ...
        f = open('%s/OutputTest_Run5-folder%d_%dns_%dkeV.pickle'%(save_folder_each, folder_index, timeWindow*1e9, energyThreshold_CsI), 'wb')
        Parameter={'nEvent': self.nEvent, "energy_exclusive":n_exclusive, "Bins":np.arange(0,2000, 20)}
        pickle.dump(Parameter, f)
        f.close()
